# DollyBot2's Check-In Log

This is an **append-only** log, read and written by the `dollybot2` Claude
Code subagent (`.claude/agents/dollybot2.md`) via scheduled email check-ins.
It's a separate, independent log from the original Dolly's
(`dolly-log/check-in-log.md`) — same format, own streak history. Don't
hand-edit the structure — only ever append new entries at the bottom,
each as its own `---`-delimited YAML block, exactly like this:
